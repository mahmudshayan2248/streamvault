...
const BITMAP_SUBTITLE_CACHE_VERSION = 'bitmap-subtitles-v2-windowed';
...
function installPlaybackCapability(options) {
  ...
  function normalizeBitmapCues(packets) {
  ...
  async function buildBitmapSubtitleManifest(source, streamIndex, windowStart = 0) {
    ...
    const task = (async () => {
      ...
      const stream = (Array.isArray(probed.streams) ? probed.streams : []).find(item => Number(item.index) === track.streamIndex)
        ...
        || {};
      const cues = normalizeBitmapCues(probed.packets);
  ...
  function renderBitmapCue(source, streamIndex, cueId, windowStart = 0) {
    return (async () => {
      ...
      const task = new Promise((resolve, reject) => {
        ...
        args.push(
          '-ss', Math.max(0, finite(cue.start)).toFixed(3),
          '-i', source.input,
          '-filter_complex', `color=c=black@0.0:s=${width}x${height}:d=${duration.toFixed(3)},format=rgba[base];[base][0:s:${manifest.relativeIndex}]overlay=format=auto,format=rgba`,
          '-frames:v', '1',
          '-update', '1',
          '-y',
          tempPath
