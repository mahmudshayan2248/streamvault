...
function normalizedSeekOrigin(packet, info, requestedStart) {
  ...
  const dts = finite(packet?.dts_time, NaN);
  ...
  function probeSeekOrigin(source, info, requestedStart) {
    ...
    const task = customProbeSeekOrigin
      ...
      : new Promise((resolve, reject) => {
          ...
          args.push(
            '-select_streams', 'v:0',
            '-read_intervals', `${requestedStart.toFixed(3)}%+#1`,
          ...
          child.on('close', code => {
            ...
            try {
              const packet = JSON.parse(stdout)?.packets?.[0];
              if (!packet) throw new Error('No seekable video keyframe found');
              finish(null, normalizedSeekOrigin(packet, info, requestedStart));
