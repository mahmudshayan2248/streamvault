...
const BITMAP_SUBTITLE_CACHE_VERSION = 'bitmap-subtitles-v5-window-prepared';
  ...
  async function renderBitmapCueWindow(source, streamIndex, windowStart = 0, preparedManifest = null) {
    ...
    const task = new Promise((resolve, reject) => {
      ...
      tempItems.forEach(({ cue }, index) => {
        const cueStart = Math.max(0, finite(cue.start) - start);
        const cueEnd = Math.max(cueStart + 0.05, Math.min(duration, finite(cue.end, cue.start + 0.5) - start));
        filters.push(`[s${index}]trim=start=${cueStart.toFixed(3)}:end=${cueEnd.toFixed(3)},setpts=PTS-STARTPTS,select='eq(n\\,0)'[out${index}]`);
      });
      const args = [
      ...
      child.on('close', code => {
        ...
        try {
          ...
          for (const item of tempItems) {
            if (fs.statSync(item.tempPath).size <= 8) throw new Error(`Bitmap subtitle window render produced an empty image for ${item.cue.id}`);
          }
