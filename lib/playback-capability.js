...
function installPlaybackCapability(options) {
  ...
  function hlsVideoCodec(decision) {
    const codec = cleanCodec(decision?.videoCodec);
    if (decision?.videoAction && decision.videoAction !== 'copy') return 'avc1.640028';
    if (H264_CODECS.has(codec)) return 'avc1.640028';
    if (HEVC_CODECS.has(codec)) return 'hvc1.2.4.L120.B0';
    if (AV1_CODECS.has(codec)) return 'av01.0.08M.08';
    return 'avc1.640028';
  }

  function hlsAudioCodec(track) {
    const codec = cleanCodec(track?.outputCodec || track?.codec);
    if (AAC_CODECS.has(codec)) return 'mp4a.40.2';
    if (MP3_CODECS.has(codec)) return 'mp4a.69';
    if (OPUS_CODECS.has(codec)) return 'opus';
    return 'mp4a.40.2';
  }
  ...
  function masterPlaylist(session) {
    ...
    const streamAttributes = [`BANDWIDTH=${Math.round(bandwidth)}`];
    const codecs = [hlsVideoCodec(session.decision), hlsAudioCodec(tracks[defaultIndex] || tracks[0])]
      .filter(Boolean)
      .filter((value, index, list) => list.indexOf(value) === index);
    if (codecs.length) streamAttributes.push(`CODECS="${codecs.join(',')}"`);
    const width = finite(session.info?.width);
    const height = finite(session.info?.height);
    if (width > 0 && height > 0) streamAttributes.push(`RESOLUTION=${Math.round(width)}x${Math.round(height)}`);
    if (tracks.length) streamAttributes.push('AUDIO="audio"');
