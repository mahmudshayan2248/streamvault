...
const BITMAP_SUBTITLE_CACHE_VERSION = 'bitmap-subtitles-v9-versioned-manifest-url';
...
function pngHasVisiblePixels(filePath) {
  const data = fs.readFileSync(filePath);
  if (data.length < 33 || data.subarray(0, 8).compare(Buffer.from([0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a])) !== 0) return false;
  let pos = 8;
  let width = 0;
  let height = 0;
  let bitDepth = 0;
  ...
  while (pos + 12 <= data.length) {
    ...
    if (chunkEnd + 4 > data.length) return false;
  ...
  for (let y = 0; y < height; y++) {
    if (offset >= inflated.length) return false;
    ...
    for (let x = 0; x < stride; x++) {
      ...
      else if (filter === 4) row[x] = (row[x] + paethPredictor(left, up, upperLeft)) & 255;
      else if (filter !== 0) return false;
    }
    for (let x = 0; x < width; x++) {
      const base = x * bytesPerPixel;
      let r = row[base], g = row[base], b = row[base], a = 255;
      if (colorType === 6) { r = row[base]; g = row[base + 1]; b = row[base + 2]; a = row[base + 3]; }
      else if (colorType === 2) { r = row[base]; g = row[base + 1]; b = row[base + 2]; }
      else if (colorType === 4) { a = row[base + 1]; }
...
function bitmapCueSampleCandidates(cue, windowStart, duration) {
  const relativeStart = Math.max(0, finite(cue.start) - windowStart);
  const relativeEnd = Math.max(relativeStart + 0.05, Math.min(duration, finite(cue.end, cue.start + 0.5) - windowStart));
  const cueDuration = Math.max(0.05, relativeEnd - relativeStart);
  const fractions = [0.5];
  const candidates = [];
  const seen = new Set();
  for (const fraction of fractions) {
    const center = relativeStart + cueDuration * fraction;
    const sampleStart = Math.max(relativeStart, Math.min(center, relativeEnd - 0.025));
    const sampleEnd = Math.max(sampleStart + 0.04, Math.min(relativeEnd, sampleStart + Math.min(0.25, Math.max(0.06, cueDuration * 0.2))));
    const key = `${sampleStart.toFixed(3)}:${sampleEnd.toFixed(3)}`;
    if (seen.has(key)) continue;
    seen.add(key);
    candidates.push({ sampleStart, sampleEnd });
  }
  return candidates;
}

function bitmapCueSampleWindow(cue, windowStart, duration) {
  return bitmapCueSampleCandidates(cue, windowStart, duration)[0] || { sampleStart: 0, sampleEnd: 0.05 };
}
...
function installPlaybackCapability(options) {
  ...
  function bitmapManifestWithUrls(manifest, source, route) {
    ...
    const query = remote
      ? `?url=${encodeURIComponent(source.input)}&start=${encodeURIComponent(String(manifest.windowStart || 0))}&v=${encodeURIComponent(manifest.cacheKey)}&bv=${encodeURIComponent(BITMAP_SUBTITLE_CACHE_VERSION)}`
      : `?start=${encodeURIComponent(String(manifest.windowStart || 0))}&v=${encodeURIComponent(manifest.cacheKey)}&bv=${encodeURIComponent(BITMAP_SUBTITLE_CACHE_VERSION)}`;
    return {
      ...manifest,
  ...
  async function renderBitmapCueWindow(source, streamIndex, windowStart = 0, preparedManifest = null) {
    const manifest = preparedManifest || await buildBitmapSubtitleManifest(source, streamIndex, windowStart);
    const cues = Array.isArray(manifest.cues) ? manifest.cues : [];
    if (!cues.length) return manifest;
    const cache = bitmapSubtitleCache(source, streamIndex, windowStart);
    fs.mkdirSync(cache.cuesDir, { recursive: true });
    const missing = cues
      .map(cue => ({ cue, finalPath: path.join(cache.cuesDir, `${cue.id}.png`) }))
      .filter(item => {
        try { return fs.statSync(item.finalPath).size <= 8 || !pngHasVisiblePixels(item.finalPath); }
        catch { return true; }
      });
    if (!missing.length) return manifest;
    const jobKey = `window-render:${cache.manifestPath}`;
    const existing = bitmapSubtitleJobs.get(jobKey);
    if (existing) return existing;
    const task = new Promise((resolve, reject) => {
      ...
      const start = Math.max(0, finite(manifest.windowStart, windowStart));
      const duration = Math.min(180, Math.max(1, finite(manifest.windowEnd, start + BITMAP_SUBTITLE_WINDOW_SECONDS) - start));
      const tempGroups = missing.map(({ cue, finalPath }) => ({
        cue,
        finalPath,
        candidates: bitmapCueSampleCandidates(cue, start, duration).map((candidate, sampleIndex) => ({
          ...candidate,
          sampleIndex,
          tempPath: `${finalPath}.${process.pid}.${crypto.randomBytes(4).toString('hex')}.sample-${sampleIndex}.tmp.png`,
        })),
      })).filter(group => group.candidates.length);
      if (!tempGroups.length) return resolve(manifest);
      const tempItems = tempGroups.flatMap(group => group.candidates.map(candidate => ({ ...candidate, cue: group.cue, finalPath: group.finalPath })));
      const labels = tempItems.map((_, index) => `[s${index}]`).join('');
      const filters = [
        `[0:v]setpts=PTS+${start.toFixed(3)}/TB[base]`,
        `[base][1:s:${manifest.relativeIndex}]overlay=format=auto,format=rgba,setpts=PTS-STARTPTS[bitmap]`,
        `[bitmap]split=${tempItems.length}${labels}`,
      ];
      tempItems.forEach((item, index) => {
        filters.push(`[s${index}]trim=start=${item.sampleStart.toFixed(3)}:end=${item.sampleEnd.toFixed(3)},setpts=PTS-STARTPTS,select='eq(n\\,0)'[out${index}]`);
      });
      const args = [
      ...
      tempItems.forEach((item, index) => {
        args.push('-map', `[out${index}]`, '-frames:v', '1', '-update', '1', '-y', item.tempPath);
      });
      ...
      const cleanup = () => {
        for (const item of tempItems) {
          try { if (fs.existsSync(item.tempPath)) fs.rmSync(item.tempPath, { force: true }); } catch {}
        }
      };
      const finish = (error, value) => {
        if (settled) return;
      ...
      const timer = setTimeout(() => {
        try { child.kill('SIGKILL'); } catch {}
        finish(new Error('Bitmap subtitle window render timed out'));
      }, 120000);
      ...
      child.on('close', code => {
        if (settled) return;
        try {
          if (code !== 0) throw new Error(`Bitmap subtitle window render failed: ${stderr.slice(-1500)}`);
          const renderedCueIds = new Set();
          const skippedCueIds = new Set();
          for (const group of tempGroups) {
            let selectedPath = null;
            for (const candidate of group.candidates) {
              try {
                if (fs.statSync(candidate.tempPath).size > 8 && pngHasVisiblePixels(candidate.tempPath)) {
                  selectedPath = candidate.tempPath;
                  break;
                }
              } catch {}
            }
            if (selectedPath) {
              try { if (fs.existsSync(group.finalPath)) fs.rmSync(group.finalPath, { force: true }); } catch {}
              fs.renameSync(selectedPath, group.finalPath);
              renderedCueIds.add(group.cue.id);
            } else {
              skippedCueIds.add(group.cue.id);
            }
          }
          cleanup();
          if (skippedCueIds.size) {
            const filteredCues = cues.filter(cue => !skippedCueIds.has(cue.id));
            const updatedManifest = {
              ...manifest,
              cues: filteredCues,
              cueCount: filteredCues.length,
              skippedCueCount: Math.max(0, finite(manifest.skippedCueCount, 0)) + skippedCueIds.size,
              blankCueIds: Array.from(new Set([...(Array.isArray(manifest.blankCueIds) ? manifest.blankCueIds : []), ...skippedCueIds])).slice(0, 50),
            };
            atomicWriteFile(cache.manifestPath, JSON.stringify(updatedManifest), 'utf8');
            return finish(null, updatedManifest);
          }
  ...
  function renderBitmapCueSample(source, manifest, cue, finalPath, sample) {
    return new Promise((resolve, reject) => {
      const width = Math.min(4096, Math.max(1, Math.round(finite(manifest.width, 1920))));
      const height = Math.min(4096, Math.max(1, Math.round(finite(manifest.height, 1080))));
      const start = Math.max(0, finite(manifest.windowStart, 0));
      const duration = Math.min(180, Math.max(1, finite(manifest.windowEnd, start + BITMAP_SUBTITLE_WINDOW_SECONDS) - start));
      ...
      args.push(
        ...
        '-i', source.input,
        '-t', duration.toFixed(3),
        '-filter_complex', `[0:v]setpts=PTS+${start.toFixed(3)}/TB[base];[base][1:s:${manifest.relativeIndex}]overlay=format=auto,format=rgba,setpts=PTS-STARTPTS[bitmap];[bitmap]trim=start=${sample.sampleStart.toFixed(3)}:end=${sample.sampleEnd.toFixed(3)},setpts=PTS-STARTPTS,select='eq(n\\,0)'[out]`,
        '-map', '[out]',
        '-frames:v', '1',
        '-update', '1',
        '-y',
      ...
      child.on('close', code => {
        ...
        try {
          ...
          if (fs.statSync(tempPath).size <= 8 || !pngHasVisiblePixels(tempPath)) return finish(null, false);
  ...
  function renderBitmapCue(source, streamIndex, cueId, windowStart = 0) {
    return (async () => {
      ...
      try {
        if (fs.statSync(finalPath).size > 8 && pngHasVisiblePixels(finalPath)) return finalPath;
      } catch {}
      ...
      const task = (async () => {
        const start = Math.max(0, finite(manifest.windowStart, windowStart));
        const duration = Math.min(180, Math.max(1, finite(manifest.windowEnd, start + BITMAP_SUBTITLE_WINDOW_SECONDS) - start));
        for (const sample of bitmapCueSampleCandidates(cue, start, duration)) {
          if (await renderBitmapCueSample(source, manifest, cue, finalPath, sample)) return finalPath;
        }
        throw new Error('Bitmap subtitle render produced no visible image');
      })().finally(() => bitmapSubtitleJobs.delete(finalPath));
  ...
  async function sendBitmapManifest(res, source, streamIndex, route, windowStart = 0) {
    try {
      const manifest = await buildBitmapSubtitleManifest(source, streamIndex, windowStart);
      const renderedManifest = await renderBitmapCueWindow(source, streamIndex, windowStart, manifest);
      res.setHeader('Access-Control-Allow-Origin', '*');
      res.setHeader('Cache-Control', 'no-store');
      res.json(bitmapManifestWithUrls(renderedManifest, source, route));
    } catch (error) {
      console.error('[Playback v2 Bitmap Subtitle] manifest failed:', error.message);
      if (!res.headersSent) { res.setHeader('Cache-Control', 'no-store'); res.status(422).json({ ok: false, code: 'BITMAP_SUBTITLE_UNAVAILABLE', error: 'Bitmap subtitle track unavailable' }); }
    }
  }
  ...
  async function sendBitmapCue(res, source, streamIndex, cueId, windowStart = 0) {
    try {
      ...
      if (!res.headersSent) { res.setHeader('Cache-Control', 'no-store'); res.status(422).json({ ok: false, code: 'BITMAP_SUBTITLE_UNAVAILABLE', error: 'Bitmap subtitle image unavailable' }); }
    }
  }
  ...
  async function capabilityFor(req, source) {
    ...
    const embedded = (Array.isArray(info.subtitleTracks) ? info.subtitleTracks : []).map((track, index) =>
      normalizedSubtitleTrack(
        ...
        streamIndex => source.remote
          ? `/api/playback/remote/bitmap-subtitles/${streamIndex}/manifest.json?url=${encodeURIComponent(source.input)}&start=${inputStart}&bv=${encodeURIComponent(BITMAP_SUBTITLE_CACHE_VERSION)}`
          : `/api/playback/${encodeURIComponent(source.id)}/bitmap-subtitles/${streamIndex}/manifest.json?start=${inputStart}&bv=${encodeURIComponent(BITMAP_SUBTITLE_CACHE_VERSION)}`
      )
    );
