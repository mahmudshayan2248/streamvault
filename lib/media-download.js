...
function normalizeRemoteUrlForCompare(value) {
  const normalized = normalizeRemoteSource({ url: value });
  return normalized ? normalized.url : '';
}

function sameRemoteUrl(left, right) {
  const a = normalizeRemoteUrlForCompare(left);
  const b = normalizeRemoteUrlForCompare(right);
  return !!a && !!b && a === b;
}
...
function createMediaDownloadResolver(options = {}) {
  ...
  function resolveRemoteUrl(query = {}) {
    const targetUrl = normalizeRemoteUrlForCompare(query.url || query.streamUrl || query.src);
    if (!targetUrl) throw new MediaDownloadError(400, 'REMOTE_SOURCE_URL_INVALID', 'Remote media source URL is invalid');

    const movieItems = (getMovieItems() || []).filter(Boolean);
    for (const movie of movieItems) {
      const sources = remoteSourcesForItem(movie);
      for (const source of sources) {
        if (!sameRemoteUrl(source.url, targetUrl)) continue;
        return {
          kind: 'remote',
          mediaType: 'movie',
          movie,
          source,
          alternateSources: sources.filter(candidate => !sameRemoteUrl(candidate.url, source.url)),
          url: source.url,
          filename: movieDownloadFilename(movie, source)
        };
      }
    }
    ...
    for (const show of (state?.shows || [])) {
      for (const [seasonKey, episodes] of Object.entries(show.seasons || {})) {
        ...
        for (const episode of episodes) {
          const candidates = [episode, ...(Array.isArray(episode.sources) ? episode.sources : [])];
          const sources = [];
          for (const candidate of candidates) {
            const source = normalizeRemoteSource(candidate, episode);
            if (!source || sources.some(existing => sameRemoteUrl(existing.url, source.url))) continue;
            sources.push(source);
          }
          for (const source of sources) {
            if (!sameRemoteUrl(source.url, targetUrl)) continue;
            const seasonNumber = Number(episode.season || episode.seasonNumber || seasonKey);
            const episodeNumber = Number(episode.episode || episode.number || episode.episodeNumber);
            return {
              kind: 'remote',
              mediaType: 'episode',
              show,
    ...
    throw new MediaDownloadError(404, 'REMOTE_SOURCE_NOT_FOUND', 'Remote media source was not found');
  }
  ...
  return { resolveIndexed, resolveMovie, resolveEpisode, resolveEpisodeById, resolveRemoteUrl };
}
