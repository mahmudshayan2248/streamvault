...
const BITMAP_SUBTITLE_CACHE_VERSION = 'bitmap-subtitles-v6-cue-midpoint-visible';
...
function bitmapCueSampleWindow(cue, windowStart, duration) {
  ...
  const sample = Math.min(relativeEnd - 0.02, relativeStart + Math.max(0.04, Math.min(cueDuration * 0.5, 0.45)));
  const sampleStart = Math.max(relativeStart, Math.min(sample, relativeEnd - 0.02));
  const sampleEnd = Math.max(sampleStart + 0.04, Math.min(relativeEnd, sampleStart + 0.25));
...
function installPlaybackCapability(options) {
  ...
  async function renderBitmapCueWindow(source, streamIndex, windowStart = 0, preparedManifest = null) {
    ...
    const missing = cues
      ...
      .filter(item => {
        try { return fs.statSync(item.finalPath).size <= 8; }
        catch { return true; }
      });
    ...
    const task = new Promise((resolve, reject) => {
      ...
      const tempItems = missing.map(({ cue, finalPath }) => ({
        cue,
        finalPath,
        tempPath: `${finalPath}.${process.pid}.${crypto.randomBytes(4).toString('hex')}.tmp.png`,
      }));
      ...
      tempItems.forEach(({ cue }, index) => {
        const { sampleStart, sampleEnd } = bitmapCueSampleWindow(cue, start, duration);
        filters.push(`[s${index}]trim=start=${sampleStart.toFixed(3)}:end=${sampleEnd.toFixed(3)},setpts=PTS-STARTPTS,select='eq(n\\,0)'[out${index}]`);
      });
      const args = [
      ...
      child.on('close', code => {
        ...
        try {
          ...
          for (const item of tempItems) {
            if (fs.statSync(item.tempPath).size <= 8 || !pngHasVisiblePixels(item.tempPath)) throw new Error(`Bitmap subtitle window render produced a blank image for ${item.cue.id}`);
          }
          for (const item of tempItems) fs.renameSync(item.tempPath, item.finalPath);
    ...
    return (async () => {
      ...
      const task = new Promise((resolve, reject) => {
        ...
        args.push(
          ...
          '-i', source.input,
          '-t', duration.toFixed(3),
          '-filter_complex', `[0:v]setpts=PTS+${start.toFixed(3)}/TB[base];[base][1:s:${manifest.relativeIndex}]overlay=format=auto,format=rgba,setpts=PTS-STARTPTS[bitmap];[bitmap]trim=start=${sampleStart.toFixed(3)}:end=${sampleEnd.toFixed(3)},setpts=PTS-STARTPTS,select='eq(n\,0)'[out]`,
          '-map', '[out]',
          '-frames:v', '1',
          '-update', '1',
          '-y',
        ...
        child.on('close', code => {
          ...
          try {
            ...
            if (fs.statSync(tempPath).size <= 8 || !pngHasVisiblePixels(tempPath)) throw new Error('Bitmap subtitle render produced a blank image');
            fs.renameSync(tempPath, finalPath);
  ...
  async function sendBitmapManifest(res, source, streamIndex, route, windowStart = 0) {
    try {
      const manifest = await buildBitmapSubtitleManifest(source, streamIndex, windowStart);
      await renderBitmapCueWindow(source, streamIndex, windowStart, manifest);
      res.setHeader('Access-Control-Allow-Origin', '*');
